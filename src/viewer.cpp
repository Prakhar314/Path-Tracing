...
Viewer::Viewer(const int width, const int height, const std::string &title)
...
void Viewer::update(glm::uvec3 **renderedOutput) {
  ...
  for (int i = 0; i < width; i++) {
    for (int j = 0; j < height; j++) {
      ...
      pixels[i + (height - 1 - j) * width] =
          SDL_MapRGB(format, pixel.r, pixel.g, pixel.b);
    }
  }
...
bool Viewer::shouldQuit() { return quit; }

...
public:
    ...
    glm::vec3 reflect(const glm::vec3 normal, const glm::vec3 incidentdirection) const override {
        ...
        glm::vec3 reflected_ray = glm::normalize(incidentdirection - 2.0f * glm::dot(incidentdirection, normal) * normal);

        return reflected_ray;
    }
    glm::vec3 reflectance(const glm::vec3 normal, const glm::vec3 incidentdirection) const override {

        //calculate incident angle
        float cosTheta = std::abs(glm::dot(normal, incidentdirection) / (glm::length(normal) * glm::length(incidentdirection)));
        float theta = std::acos(cosTheta); // in radians

        //calculate reflectance
        glm::vec3 reflectance=this->f0_reflectance+(1.0f-this->f0_reflectance)*(1-cosTheta)*(1-cosTheta)*(1-cosTheta)*(1-cosTheta)*(1-cosTheta);
        return reflectance;
    }
private: 
...
public:
    Transparent(float ri, bool met) : Material(false,met), refractive_index(ri) {}
    ...
    glm::vec3 transmit(const glm::vec3 normal, const glm::vec3 incident) const override {
            ...
            if (eta_ratio * sin_theta > 1.0) {
                ...
                glm::vec3 refracted_dir = eta_ratio * incident + (eta_ratio*cos_theta - cos_phi) * normal;
    ...
    glm::vec3 reflectance(const glm::vec3 normal, const glm::vec3 incidentdirection) const override {
        ...
        glm::vec3 f0_reflectance(0.0f);
        f0_reflectance.x=((this->refractive_index-1)*(this->refractive_index-1))/((this->refractive_index+1)*(this->refractive_index+1));
        f0_reflectance.y=((this->refractive_index-1)*(this->refractive_index-1))/((this->refractive_index+1)*(this->refractive_index+1));
        f0_reflectance.z=((this->refractive_index-1)*(this->refractive_index-1))/((this->refractive_index+1)*(this->refractive_index+1));
        ...
        //calculate reflectance
        glm::vec3 reflectance=f0_reflectance+(1.0f-f0_reflectance)*(1-cosTheta)*(1-cosTheta)*(1-cosTheta)*(1-cosTheta)*(1-cosTheta);
        return reflectance;
    }
